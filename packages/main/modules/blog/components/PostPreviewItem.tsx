...
export function PostPreviewItem({ post }: PostPreviewItemProps) {
    ...
    return (
        <article className="group grid gap-5 px-6 py-6 transition-colors hover:bg-slate-200/20 dark:hover:bg-slate-800/20 sm:grid-cols-[80px_minmax(0,1fr)] sm:grid-cols-[88px_minmax(0,1fr)_120px]">
            <Link
                href={`/blog/post/${post.slug}`}
                className="size-20 lg:size-[88px] flex hidden aspect-square shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50 text-blue-400 dark:border-slate-800 dark:bg-slate-900/70 dark:text-blue-400 sm:block"
            >
                <Preview />
            </Link>
